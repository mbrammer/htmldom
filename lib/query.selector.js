...
function getSibling(type) {
  return function(parent, node, includeComment = false) {
    let children = parent.children.filter(node => node.type === 'tag' || (includeComment ? node.type === 'comment' : false))
...
class QuerySelector {
   ...
   */
  prev (selector = '*', includeComment = false) {
    let result = new Set()

    for (let item of this) {
      let preNode = getPrevNodeSibling(this[GET_NODE_PARENT](item), item, includeComment)
   ...
   */
  next (selector = '*', includeComment = false) {
    let result = new Set()

    for (let item of this) {
      let nextNode = getNextNodeSibling(this[GET_NODE_PARENT](item), item, includeComment)
